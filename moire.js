...
function many(){
   ...
   for(var j = 0; j < h; j+=distY){
        bezierY(wy, hy, Math.cos(time)*amp, Math.sin(time)*amp,j, hslaColor(j, 100, 50, alpha));  
    }
    for(var i = 0; i < w; i+=distX){
        bezierX(wx, hx, Math.cos(time)*amp, Math.sin(time)*amp,i, hslaColor(i, 100, 50, alpha));  
